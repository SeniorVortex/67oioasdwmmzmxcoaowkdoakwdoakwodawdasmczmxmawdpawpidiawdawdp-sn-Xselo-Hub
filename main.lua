--[[
    RESENHA BATTLES - NATIVE DEBUG BUILD
    Patched from the user-provided legacy script.

    IMPORTANT:
    - Runs as a LocalScript inside your own Roblox experience.
    - Executor anti-cheat bypass / hook APIs / external loadstring utilities are intentionally disabled.
    - Original gameplay callbacks/remotes are preserved wherever they can run normally in Roblox.
...
-- Native build: executor Fix.lua bootstrap removed
...
local PlayerGuiRoot = LocalPlayer:WaitForChild("PlayerGui")

-- Troque estes IDs depois, lil rat.
local RESENHA_CONFIG = {
    Title = "RESENHA BATTLES",
    Subtitle = "DEBUG PANEL  •  PRIVATE BUILD",
    OpenIcon = "rbxassetid://137788210387292",      -- Messi beijando o cachorro
    BackgroundImage = "rbxassetid://79919983717859", -- Teto atrás de tudo
    Accent = Color3.fromRGB(255, 28, 58),
    AccentDark = Color3.fromRGB(115, 5, 25),
    Background = Color3.fromRGB(7, 7, 10),
    Surface = Color3.fromRGB(15, 15, 20),
    Surface2 = Color3.fromRGB(22, 22, 29),
    Text = Color3.fromRGB(244, 244, 249),
    Muted = Color3.fromRGB(145, 145, 160),
}

-- Force the user's Debug Battles place through the Slap Battles branch.
allowedPlaces[game.PlaceId] = true

_G.ConnectFun = _G.ConnectFun or {}

local NativeWarned = {}
local function NativeWarnOnce(key, message)
    if NativeWarned[key] then return end
    NativeWarned[key] = true
    warn("[Resenha Battles/native] " .. message)
end

-- Native fallbacks for a few executor helpers. They are deliberately conservative.
if not fireproximityprompt then
    fireproximityprompt = function(prompt)
        ...
        if not ok then
            NativeWarnOnce("proximity", "ProximityPrompt automation is not available for this object in native mode.")
        end
        return ok
    end
end

local TouchOrigins = setmetatable({}, {__mode = "k"})
if not firetouchinterest then
    firetouchinterest = function(a, b, state)
        local root = a
        local target = b
        if typeof(root) ~= "Instance" or not root:IsA("BasePart") then return false end
        if typeof(target) ~= "Instance" or not target:IsA("BasePart") then return false end
        if state == 0 then
            TouchOrigins[root] = root.CFrame
            pcall(function()
                root.CFrame = target.CFrame * CFrame.new(0, math.max(2, target.Size.Y * 0.5), 0)
            end)
        else
            local old = TouchOrigins[root]
            if old and root.Parent then
                pcall(function() root.CFrame = old end)
            end
            TouchOrigins[root] = nil
        end
        return true
    end
end
...
if not fireclickdetector then
    fireclickdetector = function(_detector, _distance)
        NativeWarnOnce("clickdetector", "fireclickdetector is executor-only. Features that depend exclusively on it need a native server-side adapter in Debug Battles.")
        return false
    end
end
...
if not getconnections then
    getconnections = function()
        NativeWarnOnce("getconnections", "getconnections is executor-only and has been replaced by an empty native result.")
        return {}
    end
end
if not firesignal then
    firesignal = function()
        NativeWarnOnce("firesignal", "firesignal is executor-only and is disabled in native mode.")
    end
end
if not setclipboard then
    setclipboard = function(text)
        NativeWarnOnce("clipboard", "Clipboard writes are unavailable from a normal LocalScript. Value: " .. tostring(text))
        return false
    end
end
if not queueonteleport then
    queueonteleport = function()
        NativeWarnOnce("queueonteleport", "queue_on_teleport is executor-only; ExecuteOnTeleport is disabled in native mode.")
    end
end
queue_on_teleport = queue_on_teleport or queueonteleport

-- Intentionally absent in native mode: hookmetamethod/newcclosure/getnamecallmethod.
-- Feature definitions guarded by these APIs simply won't be created, which is safer for a real game.
...
local function Stroke(parent, color, thickness, transparency)
    return New("UIStroke", {
        Color = color or RESENHA_CONFIG.Accent,
...
local function Tween(obj, time, props, style, direction)
    local tw = TweenService:Create(obj, TweenInfo.new(time or .16, style or Enum.EasingStyle.Quad, direction or Enum.EasingDirection.Out), props)
    tw:Play()
    return tw
end
...
local function LibConnect(signal, fn)
...
function Library:Notify(message, duration)
    local title = "Resenha Battles"
    local text = message
    local persist = false
    if type(message) == "table" then
        title = tostring(message.Title or title)
        text = tostring(message.Description or message.Text or "")
        persist = message.Persist == true
        duration = duration or message.Duration
    else
    ...
    if not self._screen then
        ...
        return {ChangeDescription=function() end, ChangeTitle=function() end, Destroy=function() end}
    end
    local holder = self._screen:FindFirstChild("ToastHolder")
    if not holder then return {ChangeDescription=function() end, ChangeTitle=function() end, Destroy=function() end} end
    ...
    Corner(toast, 10)
    Stroke(toast, RESENHA_CONFIG.Accent, 1, .35)
    local bar = New("Frame", {Size = UDim2.new(0, 3, 1, -10), Position = UDim2.fromOffset(5,5), BackgroundColor3 = RESENHA_CONFIG.Accent, BorderSizePixel = 0, ZIndex = 201}, toast)
    Corner(bar, 99)
    local titleLabel = New("TextLabel", {
        Size = UDim2.new(1, -23, 0, 18), Position = UDim2.fromOffset(17, 5), BackgroundTransparency = 1,
        Text = title, TextXAlignment = Enum.TextXAlignment.Left,
        TextColor3 = RESENHA_CONFIG.Accent, TextSize = 10, Font = Enum.Font.GothamBold, ZIndex = 201,
    }, toast)
    local descLabel = New("TextLabel", {
        Size = UDim2.new(1, -23, 1, -25), Position = UDim2.fromOffset(17, 22), BackgroundTransparency = 1,
        Text = text, TextWrapped = true, TextXAlignment = Enum.TextXAlignment.Left,
        TextColor3 = RESENHA_CONFIG.Text, TextSize = 11, Font = Enum.Font.GothamMedium, ZIndex = 201,
    }, toast)
    toast.Position = UDim2.new(1, 30, 0, 0)
    Tween(toast, .22, {Position = UDim2.new(0,0,0,0)}, Enum.EasingStyle.Back)

    local handle = {}
    function handle:ChangeDescription(v) if descLabel.Parent then descLabel.Text = tostring(v) end end
    function handle:ChangeTitle(v) if titleLabel.Parent then titleLabel.Text = tostring(v) end end
    function handle:Destroy() if toast.Parent then toast:Destroy() end end

    if not persist then
        task.delay(duration, function()
            if toast.Parent then
                Tween(toast, .18, {BackgroundTransparency = 1, Position = UDim2.new(1, 30, 0,0)}).Completed:Wait()
...
function Library:CreateLoading(_opts)
    local o = {}
    function o:SetMessage(_) end
    function o:SetDescription(_) end
    function o:ShowSidebarPage(_) end
    function o:SetCurrentStep(_) end
    function o:Continue() end
    o.Sidebar = {AddLabel = function(_, _) end}
    return o
end

local function MakeLabelObject(gui)
    local o = {Gui = gui, Value = gui.Text}
    function o:SetText(v) self.Value = tostring(v); if self.Gui then self.Gui.Text = self.Value end end
    function o:SetVisible(v) if self.Gui then self.Gui.Visible = not not v end end
    return o
end
...
local function GetControlText(id, opts)
    return tostring((opts and opts.Text) or id or "Control")
end

local function MakeControlBase(self, height)
    local row = New("Frame", {
        Size = UDim2.new(1, 0, 0, height),
        BackgroundColor3 = RESENHA_CONFIG.Surface2,
        BackgroundTransparency = .14,
        BorderSizePixel = 0,
        ZIndex = 25,
    }, self.Body)
    Corner(row, 8)
    Stroke(row, Color3.fromRGB(255,255,255), 1, .94)
    return row
end

function GroupMethods:AddLabel(text, wrap)
    local h = wrap and 38 or 25
    local row = New("TextLabel", {
        Size = UDim2.new(1,0,0,h), BackgroundTransparency = 1,
        Text = tostring(text), TextWrapped = not not wrap,
        TextXAlignment = Enum.TextXAlignment.Left, TextYAlignment = Enum.TextYAlignment.Center,
        TextColor3 = RESENHA_CONFIG.Muted, Font = Enum.Font.GothamMedium, TextSize = 11,
        ZIndex = 26,
    }, self.Body)
    return MakeLabelObject(row)
end

function GroupMethods:AddDivider()
    return New("Frame", {Size = UDim2.new(1,0,0,1), BackgroundColor3 = Color3.fromRGB(255,255,255), BackgroundTransparency = .9, BorderSizePixel = 0, ZIndex = 26}, self.Body)
end

function GroupMethods:AddButton(a, b)
    local opts, text, callback
    if type(a) == "table" then
        opts = a; text = tostring(a.Text or "Button"); callback = a.Func or a.Callback
    else
        opts = {}; text = tostring(a or "Button"); callback = b
    end
    local row = MakeControlBase(self, 38)
    row.Visible = opts.Visible ~= false
    local btn = New("TextButton", {
        Size = UDim2.new(1,0,1,0), BackgroundTransparency = 1, Text = "  "..text,
        TextXAlignment = Enum.TextXAlignment.Left, TextColor3 = RESENHA_CONFIG.Text,
        Font = Enum.Font.GothamSemibold, TextSize = 12, AutoButtonColor = false, ZIndex = 27,
    }, row)
    New("TextLabel", {AnchorPoint=Vector2.new(1,.5), Position=UDim2.new(1,-12,.5,0), Size=UDim2.fromOffset(20,20), BackgroundTransparency=1, Text="›", TextColor3=RESENHA_CONFIG.Accent, TextSize=20, Font=Enum.Font.GothamBold, ZIndex=28}, row)
    LibConnect(btn.MouseEnter, function() Tween(row,.1,{BackgroundColor3=Color3.fromRGB(38,19,26)}) end)
    LibConnect(btn.MouseLeave, function() Tween(row,.1,{BackgroundColor3=RESENHA_CONFIG.Surface2}) end)
    LibConnect(btn.MouseButton1Click, function()
        Tween(row,.06,{Size=UDim2.new(1,-4,0,36)}).Completed:Connect(function()
            if row.Parent then Tween(row,.08,{Size=UDim2.new(1,0,0,38)}) end
        end)
        if callback then task.spawn(function() local ok,err=pcall(callback); if not ok then Library:Notify("Error: "..tostring(err),4) end end) end
    end)
    local ret = {Gui=row}
    function ret:SetVisible(v) row.Visible = not not v end
    return ret
end

function GroupMethods:AddToggle(id, opts)
    opts = opts or {}
    local row = MakeControlBase(self, 42)
    row.Visible = opts.Visible ~= false
    local state = not not opts.Default
    local label = New("TextLabel", {
        Size=UDim2.new(1,-68,1,0), Position=UDim2.fromOffset(12,0), BackgroundTransparency=1,
        Text=GetControlText(id,opts), TextXAlignment=Enum.TextXAlignment.Left, TextColor3=RESENHA_CONFIG.Text,
        Font=Enum.Font.GothamSemibold, TextSize=12, ZIndex=27,
    }, row)
    local track = New("Frame", {AnchorPoint=Vector2.new(1,.5), Position=UDim2.new(1,-11,.5,0), Size=UDim2.fromOffset(42,22), BackgroundColor3=Color3.fromRGB(48,48,57), BorderSizePixel=0, ZIndex=27}, row)
    Corner(track,99)
    local knob = New("Frame", {Size=UDim2.fromOffset(16,16), Position=UDim2.fromOffset(3,3), BackgroundColor3=Color3.fromRGB(200,200,210), BorderSizePixel=0, ZIndex=28}, track)
    Corner(knob,99)
    local hit = New("TextButton", {Size=UDim2.fromScale(1,1), BackgroundTransparency=1, Text="", AutoButtonColor=false, ZIndex=29}, row)
    local obj = {Gui=row, Value=state}
    local function render(instant)
        state = obj.Value
        Tween(track, instant and 0 or .15, {BackgroundColor3 = state and RESENHA_CONFIG.Accent or Color3.fromRGB(48,48,57)})
        Tween(knob, instant and 0 or .15, {Position = state and UDim2.fromOffset(23,3) or UDim2.fromOffset(3,3), BackgroundColor3 = state and Color3.new(1,1,1) or Color3.fromRGB(200,200,210)})
    end
    function obj:SetValue(v)
        self.Value = not not v; render(false)
        if opts.Callback then task.spawn(function() local ok,err=pcall(opts.Callback,self.Value); if not ok then Library:Notify("Error in "..GetControlText(id,opts)..": "..tostring(err),4) end end) end
    end
    function obj:SetVisible(v) row.Visible = not not v end
    function obj:AddKeyPicker(keyId, kp)
        kp = kp or {}
        local keyName = tostring(kp.Default or "Unknown")
        local badge = New("TextLabel", {AnchorPoint=Vector2.new(1,.5), Position=UDim2.new(1,-58,.5,0), Size=UDim2.fromOffset(32,17), BackgroundColor3=Color3.fromRGB(31,31,39), Text="["..keyName.."]", TextColor3=RESENHA_CONFIG.Muted, TextSize=9, Font=Enum.Font.GothamBold, BorderSizePixel=0, ZIndex=28}, row)
        Corner(badge,5)
        Options[keyId] = Options[keyId] or {Value=keyName}
        local enumKey = Enum.KeyCode[keyName]
        if enumKey then
            LibConnect(UserInputService.InputBegan,function(input,gp)
                if gp or input.KeyCode ~= enumKey then return end
                if kp.Mode == "Hold" then
                    if kp.Callback then pcall(kp.Callback,true) end
                elseif kp.SyncToggleState ~= false then
                    obj:SetValue(not obj.Value)
                elseif kp.Callback then pcall(kp.Callback) end
            end)
            if kp.Mode == "Hold" then
                LibConnect(UserInputService.InputEnded,function(input)
                    if input.KeyCode == enumKey and kp.Callback then pcall(kp.Callback,false) end
                end)
            end
        end
        return obj
    end
    function obj:AddColorPicker(colorId, cp)
        cp = cp or {}
        local palette = {Color3.new(1,1,1), RESENHA_CONFIG.Accent, Color3.fromRGB(70,220,120), Color3.fromRGB(70,140,255), Color3.fromRGB(255,210,65), Color3.fromRGB(190,70,255)}
        local index, val = 1, cp.Default or palette[1]
        local swatch = New("TextButton", {AnchorPoint=Vector2.new(1,.5),Position=UDim2.new(1,-58,.5,0),Size=UDim2.fromOffset(24,20),BackgroundColor3=val,Text="",BorderSizePixel=0,AutoButtonColor=false,ZIndex=30},row)
        Corner(swatch,5)
        local option = {Value=val}
        function option:SetValue(v) self.Value=v; swatch.BackgroundColor3=v; if cp.Callback then pcall(cp.Callback,v) end end
        Options[colorId]=option
        LibConnect(swatch.MouseButton1Click,function() index=index%#palette+1; option:SetValue(palette[index]) end)
        return obj
    end
    Toggles[id] = obj
    render(true)
    LibConnect(hit.MouseButton1Click,function() obj:SetValue(not obj.Value) end)
    return obj
end

local function ShowDropdownPopup(anchor, values, current, onPick, multi)
    local screen = Library._screen
    if not screen then return end
    local old = screen:FindFirstChild("ResenhaDropdownPopup")
    if old then old:Destroy() end
    local pos = anchor.AbsolutePosition
    local size = anchor.AbsoluteSize
    local popup = New("ScrollingFrame", {
        Name="ResenhaDropdownPopup", Size=UDim2.fromOffset(math.max(180,size.X), math.min(220, 10 + #values*31)),
        Position=UDim2.fromOffset(pos.X, pos.Y+size.Y+4), BackgroundColor3=Color3.fromRGB(15,15,21), BorderSizePixel=0,
        ScrollBarThickness=2, ScrollBarImageColor3=RESENHA_CONFIG.Accent, AutomaticCanvasSize=Enum.AutomaticSize.Y,
        CanvasSize=UDim2.new(), ZIndex=500,
    }, screen)
    Corner(popup,8); Stroke(popup,RESENHA_CONFIG.Accent,1,.35); Padding(popup,5,5,5,5)
    New("UIListLayout",{Padding=UDim.new(0,4),SortOrder=Enum.SortOrder.LayoutOrder},popup)
    for _,v in ipairs(values or {}) do
        local selected = multi and type(current)=="table" and current[v] or (current==v)
        local b = New("TextButton", {Size=UDim2.new(1,0,0,27),BackgroundColor3=selected and Color3.fromRGB(55,21,30) or RESENHA_CONFIG.Surface2,BorderSizePixel=0,Text=tostring(v),TextColor3=RESENHA_CONFIG.Text,Font=Enum.Font.GothamMedium,TextSize=11,AutoButtonColor=false,ZIndex=501},popup)
        Corner(b,6)
        LibConnect(b.MouseButton1Click,function() onPick(v); if not multi and popup.Parent then popup:Destroy() end end)
    end
end

function GroupMethods:AddDropdown(id, opts)
    opts = opts or {}
    local values = opts.Values or {}
    local value = opts.Default
    if not opts.Multi and type(value) == "number" and values[value] ~= nil then
        value = values[value]
    end
    if opts.Multi then
        local normalized = {}
    ...
    local row = MakeControlBase(self, 48)
    row.Visible = opts.Visible ~= false
    New("TextLabel", {Size=UDim2.new(1,-12,0,18),Position=UDim2.fromOffset(10,3),BackgroundTransparency=1,Text=GetControlText(id,opts),TextXAlignment=Enum.TextXAlignment.Left,TextColor3=RESENHA_CONFIG.Muted,Font=Enum.Font.GothamMedium,TextSize=10,ZIndex=27},row)
    local btn = New("TextButton", {Size=UDim2.new(1,-16,0,22),Position=UDim2.fromOffset(8,22),BackgroundColor3=Color3.fromRGB(28,28,36),BorderSizePixel=0,Text="",TextColor3=RESENHA_CONFIG.Text,Font=Enum.Font.GothamMedium,TextSize=11,AutoButtonColor=false,ZIndex=27},row)
    Corner(btn,6)
    local obj = {Gui=row,Value=value,Values=values}
    local function display()
        if opts.Multi then
            local picks={}; for k,v in pairs(obj.Value or {}) do if v then table.insert(picks,tostring(k)) end end
            table.sort(picks); btn.Text = #picks>0 and table.concat(picks,", ") or "Select..."
        else
            btn.Text = tostring(obj.Value == nil or obj.Value == "" and "Select..." or obj.Value)
        end
    end
    function obj:SetValue(v)
        self.Value=v; display(); if opts.Callback then task.spawn(function() pcall(opts.Callback,self.Value) end) end
    end
    function obj:SetValues(v) self.Values=v or {}; values=self.Values; display() end
    function obj:SetVisible(v) row.Visible=not not v end
    Options[id]=obj
    display()
    LibConnect(btn.MouseButton1Click,function()
        ShowDropdownPopup(btn, obj.Values, obj.Value, function(pick)
            if opts.Multi then
                obj.Value = obj.Value or {}; obj.Value[pick] = not obj.Value[pick]; display(); if opts.Callback then task.spawn(function() pcall(opts.Callback,obj.Value) end) end
            else obj:SetValue(pick) end
        end, opts.Multi)
    end)
    return obj
end

function GroupMethods:AddSlider(id, opts)
    opts = opts or {}
    local min,max = tonumber(opts.Min) or 0, tonumber(opts.Max) or 100
    local rounding = tonumber(opts.Rounding) or 0
    local value = tonumber(opts.Default) or min
    local row = MakeControlBase(self, 55)
    New("TextLabel", {Size=UDim2.new(1,-65,0,20),Position=UDim2.fromOffset(10,4),BackgroundTransparency=1,Text=GetControlText(id,opts),TextXAlignment=Enum.TextXAlignment.Left,TextColor3=RESENHA_CONFIG.Text,Font=Enum.Font.GothamSemibold,TextSize=11,ZIndex=27},row)
    local valLabel = New("TextLabel", {AnchorPoint=Vector2.new(1,0),Position=UDim2.new(1,-10,0,4),Size=UDim2.fromOffset(52,20),BackgroundTransparency=1,TextColor3=RESENHA_CONFIG.Accent,Font=Enum.Font.GothamBold,TextSize=10,TextXAlignment=Enum.TextXAlignment.Right,ZIndex=27},row)
    local bar = New("Frame", {Size=UDim2.new(1,-20,0,8),Position=UDim2.fromOffset(10,35),BackgroundColor3=Color3.fromRGB(42,42,50),BorderSizePixel=0,ZIndex=27},row); Corner(bar,99)
    local fill = New("Frame", {Size=UDim2.fromScale(0,1),BackgroundColor3=RESENHA_CONFIG.Accent,BorderSizePixel=0,ZIndex=28},bar); Corner(fill,99)
    local hit = New("TextButton", {Size=UDim2.new(1,0,0,22),Position=UDim2.new(0,0,.5,-11),BackgroundTransparency=1,Text="",ZIndex=29},bar)
    local obj={Gui=row,Value=value}
    local function round(v)
        local p=10^rounding; return math.floor(v*p+.5)/p
    end
    local function render()
        obj.Value=math.clamp(round(obj.Value),min,max)
        valLabel.Text=tostring(obj.Value)
        fill.Size=UDim2.fromScale((obj.Value-min)/math.max(.0001,max-min),1)
    end
    function obj:SetValue(v) self.Value=tonumber(v) or self.Value; render(); if opts.Callback then task.spawn(function() pcall(opts.Callback,self.Value) end) end end
    function obj:SetVisible(v) row.Visible=not not v end
    Options[id]=obj; render()
    local dragging=false
    local function fromX(x) obj:SetValue(min+(max-min)*math.clamp((x-bar.AbsolutePosition.X)/math.max(1,bar.AbsoluteSize.X),0,1)) end
    LibConnect(hit.InputBegan,function(input) if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then dragging=true; fromX(input.Position.X) end end)
    LibConnect(UserInputService.InputChanged,function(input) if dragging and (input.UserInputType==Enum.UserInputType.MouseMovement or input.UserInputType==Enum.UserInputType.Touch) then fromX(input.Position.X) end end)
    LibConnect(UserInputService.InputEnded,function(input) if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then dragging=false end end)
    return obj
end

function GroupMethods:AddInput(id, opts)
    opts = opts or {}
    local row = MakeControlBase(self, 48)
    New("TextLabel", {Size=UDim2.new(.42,-8,1,0),Position=UDim2.fromOffset(10,0),BackgroundTransparency=1,Text=GetControlText(id,opts),TextXAlignment=Enum.TextXAlignment.Left,TextColor3=RESENHA_CONFIG.Text,Font=Enum.Font.GothamSemibold,TextSize=11,ZIndex=27},row)
    local box = New("TextBox", {AnchorPoint=Vector2.new(1,.5),Position=UDim2.new(1,-9,.5,0),Size=UDim2.new(.55,0,0,28),BackgroundColor3=Color3.fromRGB(28,28,36),BorderSizePixel=0,Text=tostring(opts.Default or ""),PlaceholderText=tostring(opts.Placeholder or ""),ClearTextOnFocus=false,TextColor3=RESENHA_CONFIG.Text,PlaceholderColor3=RESENHA_CONFIG.Muted,Font=Enum.Font.GothamMedium,TextSize=11,ZIndex=28},row); Corner(box,6)
    local obj={Gui=row,Value=box.Text}
    function obj:SetValue(v) self.Value=tostring(v or ""); box.Text=self.Value; if opts.Callback then task.spawn(function() pcall(opts.Callback, opts.Numeric and tonumber(self.Value) or self.Value) end) end end
    function obj:SetVisible(v) row.Visible=not not v end
    Options[id]=obj
    LibConnect(box.FocusLost,function() obj:SetValue(box.Text) end)
    return obj
end

function GroupMethods:AddDependencyBox()
    local dep = setmetatable({Body=self.Body}, GroupMethods)
    function dep:SetupDependencies(_deps) end
    return dep
end

function GroupMethods:SetupDependencies(_deps) end

local function MakeGroup(column, title)
    local outer = New("Frame", {Size=UDim2.new(1,-4,0,0),AutomaticSize=Enum.AutomaticSize.Y,BackgroundColor3=Color3.fromRGB(11,11,16),BackgroundTransparency=.08,BorderSizePixel=0,ZIndex=20},column)
    Corner(outer,10); Stroke(outer,Color3.fromRGB(255,255,255),1,.92); Padding(outer,10,10,9,10)
    local titleLabel = New("TextLabel", {Size=UDim2.new(1,0,0,22),BackgroundTransparency=1,Text=string.upper(tostring(title)),TextXAlignment=Enum.TextXAlignment.Left,TextColor3=RESENHA_CONFIG.Accent,Font=Enum.Font.GothamBold,TextSize=10,ZIndex=22},outer)
    local body=New("Frame",{Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y,BackgroundTransparency=1,ZIndex=22},outer)
    New("UIListLayout",{Padding=UDim.new(0,7),SortOrder=Enum.SortOrder.LayoutOrder},body)
    local obj=setmetatable({Gui=outer,Body=body,Title=titleLabel},GroupMethods)
    return obj
end

local function MakeTabbox(column)
    local tabbox={}
    function tabbox:AddTab(name) return MakeGroup(column,name) end
    return tabbox
end

function Library:CreateWindow(opts)
    opts = opts or {}
    if self._screen then self:Unload() end
    local old=PlayerGuiRoot:FindFirstChild("ResenhaBattlesNative") if old then old:Destroy() end
    local gui=New("ScreenGui",{Name="ResenhaBattlesNative",ResetOnSpawn=false,IgnoreGuiInset=true,ZIndexBehavior=Enum.ZIndexBehavior.Sibling},PlayerGuiRoot)
    self._screen=gui

    local toastHolder=New("Frame",{Name="ToastHolder",AnchorPoint=Vector2.new(1,1),Position=UDim2.new(1,-18,1,-18),Size=UDim2.fromOffset(310,280),BackgroundTransparency=1,ZIndex=199},gui)
    New("UIListLayout",{Padding=UDim.new(0,8),VerticalAlignment=Enum.VerticalAlignment.Bottom,SortOrder=Enum.SortOrder.LayoutOrder},toastHolder)

    -- Square rounded, draggable launcher. Click only fires if the drag threshold wasn't crossed.
    local launcherShadow=New("Frame",{Size=UDim2.fromOffset(66,66),Position=UDim2.new(0,22,.5,-33),BackgroundColor3=RESENHA_CONFIG.Accent,BackgroundTransparency=.67,BorderSizePixel=0,ZIndex=90},gui); Corner(launcherShadow,17)
    local launcher=New("ImageButton",{Name="Launcher",Size=UDim2.fromOffset(58,58),Position=UDim2.new(0,26,.5,-29),BackgroundColor3=Color3.fromRGB(11,11,15),BorderSizePixel=0,Image=RESENHA_CONFIG.OpenIcon,ScaleType=Enum.ScaleType.Crop,AutoButtonColor=false,ZIndex=91},gui); Corner(launcher,14); Stroke(launcher,RESENHA_CONFIG.Accent,2,.1)
    New("Frame",{AnchorPoint=Vector2.new(1,0),Position=UDim2.new(1,-3,0,3),Size=UDim2.fromOffset(8,8),BackgroundColor3=RESENHA_CONFIG.Accent,BorderSizePixel=0,ZIndex=92},launcher); Corner(launcher:FindFirstChildOfClass("Frame"),99)

    local mainShadow=New("Frame",{AnchorPoint=Vector2.new(.5,.5),Position=UDim2.fromScale(.5,.5),Size=UDim2.fromOffset(780,500),BackgroundColor3=RESENHA_CONFIG.Accent,BackgroundTransparency=.76,BorderSizePixel=0,Visible=false,ZIndex=94},gui); Corner(mainShadow,17)
    local main=New("Frame",{Name="Main",AnchorPoint=Vector2.new(.5,.5),Position=UDim2.fromScale(.5,.5),Size=UDim2.fromOffset(760,480),BackgroundColor3=RESENHA_CONFIG.Background,BorderSizePixel=0,ClipsDescendants=true,Visible=false,ZIndex=95},gui); Corner(main,14); Stroke(main,RESENHA_CONFIG.Accent,1.4,.3)
    local bg=New("ImageLabel",{Size=UDim2.fromScale(1,1),BackgroundTransparency=1,Image=RESENHA_CONFIG.BackgroundImage,ImageTransparency=.78,ScaleType=Enum.ScaleType.Crop,ZIndex=95},main)
    New("Frame",{Size=UDim2.fromScale(1,1),BackgroundColor3=Color3.fromRGB(3,3,6),BackgroundTransparency=.18,BorderSizePixel=0,ZIndex=96},main)

    local top=New("Frame",{Size=UDim2.new(1,0,0,66),BackgroundColor3=Color3.fromRGB(10,10,14),BackgroundTransparency=.04,BorderSizePixel=0,Active=true,ZIndex=100},main)
    New("Frame",{Size=UDim2.new(1,0,0,2),Position=UDim2.new(0,0,1,-2),BackgroundColor3=RESENHA_CONFIG.Accent,BackgroundTransparency=.25,BorderSizePixel=0,ZIndex=101},top)
    local title=New("TextLabel",{Size=UDim2.new(1,-170,0,30),Position=UDim2.fromOffset(20,8),BackgroundTransparency=1,Text=RESENHA_CONFIG.Title,TextXAlignment=Enum.TextXAlignment.Left,TextColor3=RESENHA_CONFIG.Accent,TextStrokeColor3=RESENHA_CONFIG.AccentDark,TextStrokeTransparency=.55,Font=Enum.Font.GothamBlack,TextSize=24,ZIndex=102},top)
    New("TextLabel",{Size=UDim2.new(1,-170,0,16),Position=UDim2.fromOffset(21,38),BackgroundTransparency=1,Text=RESENHA_CONFIG.Subtitle,TextXAlignment=Enum.TextXAlignment.Left,TextColor3=RESENHA_CONFIG.Muted,Font=Enum.Font.GothamMedium,TextSize=9,ZIndex=102},top)
    local status=New("TextLabel",{AnchorPoint=Vector2.new(1,.5),Position=UDim2.new(1,-58,.5,0),Size=UDim2.fromOffset(104,24),BackgroundColor3=Color3.fromRGB(11,27,18),BorderSizePixel=0,Text="●  NATIVE",TextColor3=Color3.fromRGB(90,255,130),Font=Enum.Font.GothamBold,TextSize=10,ZIndex=102},top); Corner(status,6)
    local close=New("TextButton",{AnchorPoint=Vector2.new(1,.5),Position=UDim2.new(1,-13,.5,0),Size=UDim2.fromOffset(32,32),BackgroundColor3=Color3.fromRGB(31,13,18),BorderSizePixel=0,Text="×",TextColor3=RESENHA_CONFIG.Accent,TextSize=22,Font=Enum.Font.GothamBold,AutoButtonColor=false,ZIndex=103},top); Corner(close,8)

    local sidebar=New("ScrollingFrame",{Size=UDim2.new(0,164,1,-66),Position=UDim2.fromOffset(0,66),BackgroundColor3=Color3.fromRGB(9,9,13),BackgroundTransparency=.1,BorderSizePixel=0,ScrollBarThickness=0,AutomaticCanvasSize=Enum.AutomaticSize.Y,CanvasSize=UDim2.new(),ZIndex=99},main)
    Padding(sidebar,8,8,10,10); New("UIListLayout",{Padding=UDim.new(0,6),SortOrder=Enum.SortOrder.LayoutOrder},sidebar)
    local content=New("Frame",{Size=UDim2.new(1,-164,1,-66),Position=UDim2.fromOffset(164,66),BackgroundTransparency=1,ZIndex=99},main)

    local window={Gui=main,TitleLabel=title,Tabs={},Selected=nil}
    self._window=window
    function window:ChangeTitle(v) title.Text=tostring(v) end
    function window:AddTab(name,_icon)
        local button=New("TextButton",{Size=UDim2.new(1,0,0,38),BackgroundColor3=Color3.fromRGB(45,18,25),BackgroundTransparency=1,BorderSizePixel=0,Text="   "..tostring(name),TextXAlignment=Enum.TextXAlignment.Left,TextColor3=RESENHA_CONFIG.Muted,Font=Enum.Font.GothamSemibold,TextSize=11,AutoButtonColor=false,ZIndex=101},sidebar); Corner(button,7)
        local indicator=New("Frame",{Size=UDim2.fromOffset(3,20),Position=UDim2.new(0,0,.5,-10),BackgroundColor3=RESENHA_CONFIG.Accent,BackgroundTransparency=1,BorderSizePixel=0,ZIndex=102},button); Corner(indicator,99)
        local page=New("Frame",{Size=UDim2.fromScale(1,1),BackgroundTransparency=1,Visible=false,ZIndex=100},content)
        local left=New("ScrollingFrame",{Size=UDim2.new(.5,-7,1,-16),Position=UDim2.fromOffset(8,8),BackgroundTransparency=1,BorderSizePixel=0,ScrollBarThickness=2,ScrollBarImageColor3=RESENHA_CONFIG.Accent,AutomaticCanvasSize=Enum.AutomaticSize.Y,CanvasSize=UDim2.new(),ZIndex=100},page); Padding(left,0,5,0,8); New("UIListLayout",{Padding=UDim.new(0,9),SortOrder=Enum.SortOrder.LayoutOrder},left)
        local right=New("ScrollingFrame",{Size=UDim2.new(.5,-7,1,-16),Position=UDim2.new(.5,0,0,8),BackgroundTransparency=1,BorderSizePixel=0,ScrollBarThickness=2,ScrollBarImageColor3=RESENHA_CONFIG.Accent,AutomaticCanvasSize=Enum.AutomaticSize.Y,CanvasSize=UDim2.new(),ZIndex=100},page); Padding(right,5,0,0,8); New("UIListLayout",{Padding=UDim.new(0,9),SortOrder=Enum.SortOrder.LayoutOrder},right)
        local tab={Button=button,Page=page,Left=left,Right=right}
        function tab:AddLeftGroupbox(t) return MakeGroup(left,t) end
        function tab:AddRightGroupbox(t) return MakeGroup(right,t) end
        function tab:AddLeftTabbox() return MakeTabbox(left) end
        function tab:AddRightTabbox() return MakeTabbox(right) end
        local function select()
            for _,t in pairs(window.Tabs) do
                t.Page.Visible=false; Tween(t.Button,.12,{BackgroundTransparency=1,TextColor3=RESENHA_CONFIG.Muted}); local ii=t.Button:FindFirstChildOfClass("Frame"); if ii then ii.BackgroundTransparency=1 end
            end
            page.Visible=true; Tween(button,.12,{BackgroundTransparency=.15,TextColor3=RESENHA_CONFIG.Text}); indicator.BackgroundTransparency=0; window.Selected=tab
        end
        LibConnect(button.MouseButton1Click,select)
    ...
    local function setOpen(v)
        ...
        if open then
            ...
            main.Size=UDim2.fromOffset(700,440); mainShadow.Size=UDim2.fromOffset(720,460); main.BackgroundTransparency=.4
            Tween(main,.22,{Size=UDim2.fromOffset(760,480),BackgroundTransparency=0},Enum.EasingStyle.Back)
            Tween(mainShadow,.22,{Size=UDim2.fromOffset(780,500)},Enum.EasingStyle.Back)
        else
            Tween(main,.15,{Size=UDim2.fromOffset(710,445),BackgroundTransparency=1})
            Tween(mainShadow,.15,{Size=UDim2.fromOffset(730,465),BackgroundTransparency=1})
            task.delay(.16,function() if not open and main.Parent then main.Visible=false;mainShadow.Visible=false;main.Size=UDim2.fromOffset(760,480);mainShadow.Size=UDim2.fromOffset(780,500);main.BackgroundTransparency=0;mainShadow.BackgroundTransparency=.76 end end)
        end
    end
    LibConnect(close.MouseButton1Click,function() setOpen(false) end)
    LibConnect(UserInputService.InputBegan,function(input,gp) if not gp and input.KeyCode==Enum.KeyCode.RightShift then setOpen(not open) end end)

    -- Launcher drag/click with movement threshold.
    local dragging=false; local dragStart; local launcherStart; local shadowStart; local moved=false
    LibConnect(launcher.InputBegan,function(input)
        if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then
            dragging=true;moved=false;dragStart=input.Position;launcherStart=launcher.Position;shadowStart=launcherShadow.Position
        end
    end)
    LibConnect(UserInputService.InputChanged,function(input)
        if not dragging then return end
        if input.UserInputType~=Enum.UserInputType.MouseMovement and input.UserInputType~=Enum.UserInputType.Touch then return end
        local d=input.Position-dragStart
        if d.Magnitude>6 then moved=true end
        launcher.Position=UDim2.new(launcherStart.X.Scale,launcherStart.X.Offset+d.X,launcherStart.Y.Scale,launcherStart.Y.Offset+d.Y)
        launcherShadow.Position=UDim2.new(shadowStart.X.Scale,shadowStart.X.Offset+d.X,shadowStart.Y.Scale,shadowStart.Y.Offset+d.Y)
    end)
    LibConnect(UserInputService.InputEnded,function(input)
        if dragging and (input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch) then
            dragging=false
            if not moved then setOpen(not open) end
        end
    end)

    -- Main titlebar drag.
    local md=false;local ms;local mp
    LibConnect(top.InputBegan,function(input) if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then md=true;ms=input.Position;mp=main.Position end end)
    LibConnect(UserInputService.InputChanged,function(input) if md and (input.UserInputType==Enum.UserInputType.MouseMovement or input.UserInputType==Enum.UserInputType.Touch) then local d=input.Position-ms; local np=UDim2.new(mp.X.Scale,mp.X.Offset+d.X,mp.Y.Scale,mp.Y.Offset+d.Y); main.Position=np;mainShadow.Position=np end end)
    LibConnect(UserInputService.InputEnded,function(input) if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then md=false end end)

    return window
end

-- ESP compatibility: no executor Drawing API in a normal game client. Keep calls safe.
local ESPLibrary = {}
for _,method in ipairs({"AddESP","RemoveESP","SetArrowRadius","SetArrows","SetFillTransparency","SetFont","SetOutlineColor","SetOutlineTransparency","SetRainbow","SetShowDistance","SetTextSize","SetTracerOrigin","SetTracers","UpdateObjectColor","UpdateObjectText"}) do
    ESPLibrary[method] = function() return nil end
end
for _,method in ipairs({"AddESP","RemoveESP","SetArrowRadius","SetArrows","SetFillTransparency","SetFont","SetOutlineColor","SetOutlineTransparency","SetRainbow","SetShowDistance","SetTextSize","SetTracerOrigin","SetTracers","UpdateObjectColor","UpdateObjectText","SetDPIScale"}) do
    Library[method] = function() return nil end
end
...
if place and type(place) == "string" then
    local yasss = pcall(function()
		error("Native Debug Battles build: executor anti-cheat bypass disabled")
	end)
    if yasss then
        successbypass = true
    end
...
if PlayerGuiRoot:FindFirstChild("Cooldown Script") == nil then
local gui = Create("ScreenGui", {Name = "Cooldown Script", IgnoreGuiInset = true, Parent = PlayerGuiRoot})
...
function Cooldown(time, text)
	if PlayerGuiRoot:FindFirstChild("Cooldown Script") then
		if PlayerGuiRoot["Cooldown Script"].ImageLabel.Visible then return end
		spawn(function()
			local BarTween = TweenService:Create(PlayerGuiRoot["Cooldown Script"].ImageLabel.Frame, TweenInfo.new(time, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {Size = UDim2.new(0.001, 0, 0.98, 0)})
			BarTween:Play()
			BarTween.Completed:Wait()
			PlayerGuiRoot["Cooldown Script"].ImageLabel.Visible = false
			PlayerGuiRoot["Cooldown Script"].ImageLabel.Frame.Size = UDim2.new(1, 0, 0.98, 0)
			PlayerGuiRoot["Cooldown Script"].ImageLabel.TextLabel.Rotation = 15
			TweenService:Create(PlayerGuiRoot["Cooldown Script"].ImageLabel, TweenInfo.new(0.06, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {Position = UDim2.new(1.01, 0, 0.305, -50)}):Play()
		end)
		spawn(function()
			PlayerGuiRoot["Cooldown Script"].ImageLabel.TextLabel.Text = text
			PlayerGuiRoot["Cooldown Script"].ImageLabel.Visible = true
			TweenService:Create(PlayerGuiRoot["Cooldown Script"].ImageLabel, TweenInfo.new(0.2, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Position = UDim2.new(0.98, 0, 0.3, -50)}):Play()
			SpringText(PlayerGuiRoot["Cooldown Script"].ImageLabel.TextLabel, 20)
		end)
	end
end
function CheckCooldown()
	return PlayerGuiRoot:FindFirstChild("Cooldown Script") and PlayerGuiRoot["Cooldown Script"]:FindFirstChild("ImageLabel") and PlayerGuiRoot["Cooldown Script"].ImageLabel.Visible
end
if PlayerGuiRoot:FindFirstChild("Gui Button Script") == nil then
local gui = Create("ScreenGui", {Name = "Gui Button Script", Enabled = false, Parent = PlayerGuiRoot})
...
if not PlayerGuiRoot:FindFirstChild("Keybind Script") then
local gui = Create("ScreenGui", {Name = "Keybind Script", Enabled = false, IgnoreGuiInset = true, Parent = PlayerGuiRoot})
...
table.insert(_G.ConnectFun, UserInputService.InputBegan:Connect(function(input, gameProcessed)
	...
	if CheckCooldown() and (PlayerGuiRoot:FindFirstChild("Keybind Script") and PlayerGuiRoot:FindFirstChild("Keybind Script").Enabled) or (PlayerGuiRoot:FindFirstChild("Gui Roll Script") and not PlayerGuiRoot:FindFirstChild("Gui Roll Script").Enabled) then return end
	if input.KeyCode == edit.Key then
...
function ShowButtonU(show)
	if MobileOn then
		if PlayerGuiRoot:FindFirstChild("Gui Button Script") then
			PlayerGuiRoot:FindFirstChild("Gui Button Script").Enabled = show
		end
	else
		if PlayerGuiRoot:FindFirstChild("Keybind Script") then
			PlayerGuiRoot:FindFirstChild("Keybind Script").Enabled = show
		end
	end
end

if not PlayerGuiRoot:FindFirstChild("Gui Track") then
    local gui = Create("ScreenGui", {Name = "Gui Track", Enabled = false, Parent = PlayerGuiRoot})
    local Frame = Create("Frame", {
...
function GuiTrackUpdate(enabled, update)
	...
	if not enabledGui then
		PlayerGuiRoot["Gui Track"].Frame:FindFirstChild("TextLabel").Text = "Nah"
		PlayerGuiRoot["Gui Track"].Frame.Frame.Frame1.Size = UDim2.new(0, 0, 1, 0)
	end
	if PlayerGuiRoot:FindFirstChild("Gui Track") then
		PlayerGuiRoot["Gui Track"].Enabled = enabledGui
		if PlayerGuiRoot["Gui Track"].Enabled then
			PlayerGuiRoot["Gui Track"].Frame:FindFirstChild("TextLabel").Text = update.Text
			local TweenBar = game:GetService("TweenService"):Create(PlayerGuiRoot["Gui Track"].Frame.Frame.Frame1, TweenInfo.new(1.5), {Size = UDim2.new(update.Size, 0, 1, 0)})
			TweenBar:Play()
		end
	end
end
...
if not PlayerGuiRoot:FindFirstChild("MasteryGui") then
	Create("ScreenGui", {
	    Name = "MasteryGui",
	    ZIndexBehavior = Enum.ZIndexBehavior.Sibling,
	    Parent = PlayerGuiRoot
	}, {
	    Create("Frame", {
	        Name = "Main",
	        Active = true,
...
local TweenService = game:GetService("TweenService")
local masteryGloveGui = PlayerGuiRoot:FindFirstChild("MasteryGui")
...
task.wait(1)
-- ESPLibrary supplied by Resenha native compatibility layer
...
local wminfo = { setText = function(_) end }
...
Script1Group:AddButton({
    ...
    Func = function()
Library:Notify("External executor script disabled in native Debug Battles build", 4)
    end
})
...
for i,v in pairs(PlayerGuiRoot.RobloxPromptGui.promptOverlay:GetDescendants()) do
...
if PlayerGuiRoot:FindFirstChild("SlapRock") == nil then
local ScreenGui = Instance.new("ScreenGui")
local Button = Instance.new("ImageButton")
local Corner = Instance.new("UICorner")
local ParticleEmitter = Instance.new("ParticleEmitter")

ScreenGui.Parent = PlayerGuiRoot
...
Library:OnUnload(function()
	...
	if PlayerGuiRoot.RobloxGui.Backpack:FindFirstChild("Hotbar") then
		PlayerGuiRoot.RobloxGui.Backpack.Hotbar.Position = _G.Backpack["Old"]["Hotbar"]
	end
	if PlayerGuiRoot.RobloxGui.Backpack:FindFirstChild("Inventory") then
		PlayerGuiRoot.RobloxGui.Backpack.Inventory.Position = _G.Backpack["Old"]["Inventory"]
	end
	for i, v in pairs(PlayerGuiRoot.RobloxGui.Backpack.Hotbar:GetChildren()) do
		if v:IsA("TextButton") and v:FindFirstChild("Number") then
			v.Number.Position = _G.Backpack["Old"]["Backpack Number"][v.Name]
		end
	end
	if PlayerGuiRoot.RobloxGui.Backpack.Hotbar:FindFirstChild("UIListLayout") then
		PlayerGuiRoot.RobloxGui.Backpack.Hotbar:FindFirstChild("UIListLayout"):Destroy()
	end
	for i, v in pairs(PlayerGuiRoot.RobloxGui.Backpack.Hotbar:GetChildren()) do
	...
	for i, v in pairs(PlayerGuiRoot.RobloxGui.Backpack.Inventory.ScrollingFrame.UIGridFrame:GetChildren()) do
		if v:IsA("TextButton") then
			if v:FindFirstChild("Equipped") then
				v.Equipped.Visible = true
			end
	...
	for i, v in pairs(PlayerGuiRoot.RobloxGui.Backpack.Inventory:GetChildren()) do
	...
	if PlayerGuiRoot.RobloxGui.Backpack:FindFirstChild("Inventory") and PlayerGuiRoot.RobloxGui.Backpack.Inventory:FindFirstChild("UICorner") then
		PlayerGuiRoot.RobloxGui.Backpack.Inventory:FindFirstChild("UICorner"):Destroy()
	end
	_G.Backpack = nil
	gloveHits = nil
	if AutoSetInfoServer then
...
_G.Backpack = {
	["Old"] = {
		["Hotbar"] = PlayerGuiRoot.RobloxGui.Backpack.Hotbar.Position,
		["Inventory"] = PlayerGuiRoot.RobloxGui.Backpack.Inventory.Position,
		["Backpack Number"] = {}
	},
	["New"] = {
		["Hotbar"] = (PlayerGuiRoot.RobloxGui.Backpack.Hotbar.Position + UDim2.new(0, 0, 0, -10)),
		["Inventory"] = (PlayerGuiRoot.RobloxGui.Backpack.Inventory.Position + UDim2.new(0, 0, 0, -10)),
		["Backpack Number"] = {}
	}
}
wait()
...
if PlayerGuiRoot.RobloxGui.Backpack:FindFirstChild("Hotbar") then
PlayerGuiRoot.RobloxGui.Backpack.Hotbar.Position = (_G.BackpackV2 == true and (_G.Backpack["New"]["Hotbar"]) or (_G.Backpack["Old"]["Hotbar"]))
end
if PlayerGuiRoot.RobloxGui.Backpack:FindFirstChild("Inventory") then
PlayerGuiRoot.RobloxGui.Backpack.Inventory.Position = (_G.BackpackV2 == true and (_G.Backpack["New"]["Inventory"]) or (_G.Backpack["Old"]["Inventory"]))
end
for i, v in pairs(PlayerGuiRoot.RobloxGui.Backpack.Hotbar:GetChildren()) do
if v:IsA("TextButton") and v:FindFirstChild("Number") then
...
if PlayerGuiRoot.RobloxGui.Backpack:FindFirstChild("Inventory").Visible == false then
if PlayerGuiRoot.RobloxGui.Backpack:FindFirstChild("Hotbar"):FindFirstChild("UIListLayout") == nil then
...
UIListLayout.Parent = PlayerGuiRoot.RobloxGui.Backpack.Hotbar
end
else
if PlayerGuiRoot.RobloxGui.Backpack.Hotbar:FindFirstChild("UIListLayout") then
PlayerGuiRoot.RobloxGui.Backpack.Hotbar:FindFirstChild("UIListLayout"):Destroy()
...
if PlayerGuiRoot.RobloxGui.Backpack:FindFirstChild("Inventory") and PlayerGuiRoot.RobloxGui.Backpack.Inventory:FindFirstChild("UICorner") == nil then
local RobloxUi = Instance.new("UICorner", PlayerGuiRoot.RobloxGui.Backpack.Inventory)
RobloxUi.CornerRadius = UDim.new(0, 10)
end
...
table.insert(_G.ConnectFun, PlayerGuiRoot.RobloxGui.Backpack.Inventory.ScrollingFrame.UIGridFrame.ChildAdded:Connect(function(v)
if v:IsA("TextButton") then
BackpackV2(v)
end
end))
